...
class FunSetSuite {
  ...
  @Test def `exists a>0`: Unit = {
    new TestSets {
      val s = (union(union(s1, s2), s3))
      assertEquals(true, exists(s, x => x > 0))
    }
  }

  @Test def `exists a>1`: Unit = {
    new TestSets {
      val s = (union(union(s1, s2), s3))
      assertEquals(true, exists(s, x => x > 1))
    }
  }

  @Test def `exists a == 2`: Unit = {
    new TestSets {
      val s = union(s1, s3)
      assertEquals(false, exists(s, x => x == 2))
    }
  }
